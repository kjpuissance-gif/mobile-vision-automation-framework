#!/bin/bash
# ╔══════════════════════════════════════════════════════════════════════════════╗
# ║  THE GAMER IA GOD – INSTALLATION 1-CLICK                                    ║
# ║  Crée l'environnement, installe tout, télécharge le modèle Tiny, lance le bot║
# ╚══════════════════════════════════════════════════════════════════════════════╝

set -e  # Arrêt en cas d'erreur
trap 'echo -e "\n❌ Erreur à la ligne $LINENO. Vérifie ta connexion internet et réessaie."' ERR
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║   THE GAMER IA GOD – Installation automatique                 ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"

# 1. Créer le dossier du projet
PROJECT_DIR="$HOME/gamer_ia_god"
mkdir -p "$PROJECT_DIR"
cd "$PROJECT_DIR"
echo -e "${GREEN}📁 Dossier de travail : $PROJECT_DIR${NC}"

# 2. Installer les dépendances système
echo -e "${GREEN}📦 Mise à jour système et installation des paquets...${NC}"
sudo apt-get update -qq
sudo apt-get install -y -qq python3 python3-pip python3-venv git wget adb

# 3. Créer l'environnement virtuel
echo -e "${GREEN}🐍 Création de l'environnement virtuel...${NC}"
python3 -m venv god_env
source god_env/bin/activate

# 4. Installer les dépendances Python (y compris PyTorch CPU)
echo -e "${GREEN}📦 Installation des paquets Python (cela peut prendre quelques minutes)...${NC}"
pip install --upgrade pip -q
pip install opencv-python numpy ultralytics easyocr groq -q
pip install torch torchvision --index-url https://download.pytorch.org/whl/cpu -q

# 5. Installation de Grounding DINO
echo -e "${GREEN}🌍 Installation de Grounding DINO...${NC}"
...
# 6. Télécharger le modèle pré-entraîné Tiny (environ 700 Mo)
echo -e "${GREEN}⬇️ Téléchargement du modèle Grounding DINO Tiny...${NC}"
mkdir -p weights
...
# 7. Créer le fichier de configuration pour les chemins
cat > config_paths.py << EOF
GROUNDING_CONFIG = "$PROJECT_DIR/GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py"
GROUNDING_WEIGHTS = "$PROJECT_DIR/weights/groundingdino_swint_ogc.pth"
EOF

# 8. Générer le script Python du bot (version compacte avec adaptation des chemins)
cat > divine_bot.py << 'EOF'
#!/usr/bin/env python3
# THE GAMER IA GOD v13.0 (installé automatiquement)
import subprocess, time, os, sys, threading, random, re, numpy as np
from collections import deque
import logging

# Chemins des modèles (lues depuis config_paths.py)
try:
    from config_paths import GROUNDING_CONFIG, GROUNDING_WEIGHTS
except:
...
class ADB:
    ...
    def swipe(self, x1, y1, x2, y2, duration=100):
        ...
        time.sleep(0.1)
    def capture(self, path="/tmp/screen.png"):
        try:
...
class Vision:
    def __init__(self):
        ...
        if GROUNDING_AVAILABLE and CV2_AVAILABLE:
            try:
                self.grounding = load_model(GROUNDING_CONFIG, GROUNDING_WEIGHTS, device="cpu")
        ...
        if self.grounding and (force or now - self.last_full > 30):
            try:
                ...
                for i, box in enumerate(boxes):
                    ...
                    enemies.append({"x":x1,"y":y1,"w":x2-x1,"h":y2-y1,"center":((x1+x2)//2,(y1+y2)//2),"conf":float(logits[i]),"area":(x2-x1)*(y2-y1)})
                if enemies:
                    self.cache, self.last_full = enemies, now
                    return enemies
            except Exception as e: self.logger.error(f"Grounding error: {e}")
        if self.yolo:
            try:
                    ...
                    if r.boxes is not None:
                        for box in r.boxes:
                            if int(box.cls[0]) == 0:
                                ...
                                enemies.append({"x":x1,"y":y1,"w":x2-x1,"h":y2-y1,"center":((x1+x2)//2,(y1+y2)//2),"conf":float(box.conf[0]),"area":(x2-x1)*(y2-y1)})
                if enemies:
                    self.cache = enemies
                    return enemies
            except: pass
        if self.cache and self.kalman:
            pred = []
            for e in self.cache:
                cx,cy = e["center"]
                ...
                pred.append(ne)
            return pred
        return self.cache if self.cache else self._skin(frame)
        ...
        try:
            ...
            for c in cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)[0]:
                if cv2.contourArea(c) > 500:
                    ...
                    enemies.append({"x":x,"y":y,"w":w,"h":h,"center":(x+w//2,y+h//2),"conf":1.0,"area":w*h})
            return enemies
        except: return []
...
class Bot:
    def __init__(self, device_id=None):
        ...
        self.chat = self._chat if self.groq else None
        self.decide = self._decide_groq if self.groq else self._decide_fallback
    ...
    def _decide_groq(self, order=None):
        prompt = f"HP={self.state['hp']:.0f}% Ammo={self.state['ammo']:.0f}% Ennemis={len(self.state['enemies'])}. Actions récentes={list(self.state['last_actions'])}. "
    ...
    def run(self):
        ...
        cycle = 0
        while self.running:
            frame = self.adb.capture()
            if frame is not None:
                self.state["enemies"] = self.vision.detect(frame, force=(cycle%10==0))
                self.state["hp"] = self.vision.hp(frame)
                self.state["ammo"] = self.vision.ammo(frame)
            self.logger.info(f"Cycle {cycle}: HP={self.state['hp']:.0f}% Ammo={self.state['ammo']:.0f}% Ennemis={len(self.state['enemies'])}")
            if self.user_order:
                a,r = self.decide(self.user_order)
                self.user_order = None
            else:
                a,r = self.decide()
            self._execute(a, r)
...
def main():
    ...
    print("\n🧠 THE GAMER IA GOD v13.0 – Prêt à tout installer\n")
    Bot(args.device).run()
...
# 9. Rendre le bot exécutable
chmod +x divine_bot.py

# 10. Récupérer la clé Groq (optionnelle)
echo -e "${BLUE}🔑 Souhaites-tu entrer une clé API Groq pour la conscience ? (sinon, bot mode dégradé) [o/N]${NC}"
read -r answer
if [[ "$answer" =~ ^[OoYy] ]]; then
    echo -e "${BLUE}Entrez votre clé :${NC}"
    read -rs groq_key
    echo "$groq_key" > .api_key
    echo -e "${GREEN}Clé sauvegardée dans .api_key${NC}"
else
    echo -e "${RED}Pas de clé → bot sans dialogue ni IA décisionnelle (règles simples)${NC}"
fi

# 11. Vérifier que le téléphone est connecté
echo -e "${GREEN}📱 Vérification de la connexion ADB...${NC}"
adb devices

# 12. Lancer le bot
echo -e "${GREEN}🚀 Lancement du bot...${NC}"
source god_env/bin/activate
python divine_bot.py "$@"